...
class CameraManager(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self.placeholder_im = Image.open(get_resource_path("assets/images/placeholder.png"))
        self.placeholder_im = np.array(self.placeholder_im.convert('RGB'))

        # Overlays
        self.overlay_active = cv2.cvtColor(
            cv2.imread(get_resource_path("assets/images/overlays/active.png"),
                       cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGB)
        self.overlay_disabled = cv2.cvtColor(
            cv2.imread(get_resource_path("assets/images/overlays/disabled.png"),
                       cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGB)
        self.overlay_face_not_detected = cv2.cvtColor(
            cv2.imread(get_resource_path("assets/images/overlays/face_not_detected.png"),
                       cv2.IMREAD_UNCHANGED), cv2.COLOR_BGRA2RGB)
    ...
    def draw_overlay(self, track_loc):
        ...
        # Active - Cache config call
        config = ConfigManager().config
        if config["use_transformation_matrix"]:
            cx = int(config["fix_width"]) // 2
            cy = int(config["fix_height"]) // 2
        ...
        while not stop_flag.is_set():
            try:
                ...
                if not self.assign_done_flag.is_set():
                    time.sleep(0.01)  # Reducido de 0.5 a 0.01 segundos
                    continue
                ...
                # Trim image - Cache config
                config = ConfigManager().config
                if h != config["fix_height"] or w != config["fix_width"]:
                    ...
                    frame = cv2.resize(frame, (config["fix_width"], config["fix_height"]))
                frame = cv2.flip(frame, 1)
                
                # Diagnostic logging of frame intensity
                if not hasattr(self, "_frame_count"):
                    self._frame_count = 0
                self._frame_count += 1
                if self._frame_count % 100 == 0:
                    logger.info(f"[DIAG_CAM] Active camera {self.curr_id} frame mean brightness: {frame.mean():.2f}")
                
                # Make read-only for MediaPipe mesh performance
                try:
                ...
                logger.error(f"Error in read_camera_loop: {e}", exc_info=e)
                time.sleep(0.05)

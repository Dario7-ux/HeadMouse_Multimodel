...
from src.gui.theme_colors import (
    PROFILE_DIV_DEFAULT, PROFILE_DIV_HOVER, PROFILE_DIV_SELECTED,
    BG_CARD, ACCENT, SUCCESS_BG, DANGER_BG, TEXT_PRIMARY, get_color
)
...
DIV_COLORS = {
    "default": PROFILE_DIV_DEFAULT,
    "hovering": PROFILE_DIV_HOVER,
    "selected": PROFILE_DIV_SELECTED
}
...
class FrameProfileItems(SafeDisposableScrollableFrame):
    ...
    def hover_enter(self, div, event):
        ...
        for widget_name, widget in div.items():
            ...
            if widget_name in target_widgets:
                widget.configure(fg_color=get_color(DIV_COLORS["hovering"]))
    ...
    def hover_leave(self, div, event):
        ...
        for widget_name, widget in div.items():
            ...
            if div["is_editing"]:
                target_widgets.remove("entry")
            if widget_name in target_widgets:
                widget.configure(fg_color=get_color(PROFILE_DIV_DEFAULT))
    ...
    def set_div_selected(self, div: dict, event=None):
        ...
        for widget_name, widget in div.items():
            ...
            if div["is_editing"]:
                target_widgets.remove("entry")
            if widget_name in target_widgets:
                widget.configure(fg_color=get_color(DIV_COLORS["selected"]))
    ...
    def rename_button_callback(self, div: dict):
        ...
        div["entry"].configure(state="normal",
                               border_width=2,
                               border_color=get_color(SUCCESS_BG),
    ...
    def check_profile_name_valid(self, div, var, index, mode):
        ...
        # Change border color
        if is_valid_input:
            div["entry"].configure(border_width=2, border_color=get_color(SUCCESS_BG))
        else:
            div["entry"].configure(border_width=2, border_color=get_color(DANGER_BG))
...
class FrameProfile(SafeDisposableFrame):
    ...
    def __init__(self, master, refresh_master_fn: callable, **kwargs):
        ...
        self.float_window.configure(fg_color=get_color(BG_CARD))
